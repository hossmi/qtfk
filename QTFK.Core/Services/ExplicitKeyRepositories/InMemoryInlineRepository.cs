...
{
    public class InMemoryInlineRepository<TKey, TItem> : InMemoryRepositoryBase<TKey, TItem> where TItem : new()
    {
        ...
        public InMemoryInlineRepository(
            ...
            ) : base(items)
        {
            ...
            _generateNewID = item => {
                throw new TypeInitializationException(this.GetType().FullName
                    , new Exception($"This {nameof(InMemoryInlineRepository<TKey, TItem>)} has been initialized without {nameof(GenerateNewID)} function, but it has been called. Possible bug? O__o"));
            };
        }
